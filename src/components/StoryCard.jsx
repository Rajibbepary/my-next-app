...
const StoryCard = ({story}) => {
    const { name, image, company} = story
    return (
          <div className="flex flex-wrap flex-row gap-8">
        
            <div className="bg-green-200 rounded-2xl pb-4 overflow-hidden border border-gray-200 hover:-translate-y-1 transition duration-300">
                <div className="w-64 flex justify-center pt-10">
                    <div className="w-28 h-28 rounded-full overflow-hidden">
                        <img className="h-32 object-cover object-top" src={image} alt="userImage2" />
                    </div>
                </div>
                <div className="flex flex-col items-center">
                    <p className="font-medium mt-3 text-gray-600">{name}</p>
                    <p className="text-gray-500 text-sm">{company}</p>
                   <Link href={`/stories/${story.id}`}> <button  className="border text-sm text-gray-500 border-gray-200 hover:bg-gray-100 transition cursor-pointer px-6 py-1 rounded-full mt-5 flex items-center justify-center gap-1">
